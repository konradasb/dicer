...
func TestInstanceMounts(t *testing.T) {
	...
	if got := strings.TrimSpace(env.exec(t, name, "cat", "/etc/dicer-e2e.conf")); got != "key=value" {
		t.Errorf("/etc/dicer-e2e.conf = %q, want %q", got, "key=value")
	}
	if _, err := env.tryExec(t, name, "sh", "-c", "echo x > /etc/dicer-e2e.conf"); err == nil {
		t.Error("writing to a read-only file mount succeeded")
	}

	// The tmpfs is mounted before the file inside it, which stays writable.
	env.exec(t, name, "sh", "-c", "echo changed > /scratch/nested/copy.conf && touch /scratch/other")
	if got := strings.TrimSpace(env.exec(t, name, "cat", "/scratch/nested/copy.conf")); got != "changed" {
		t.Errorf("/scratch/nested/copy.conf = %q, want the guest's write", got)
	}
}
