...
`dicerd` reads its configuration from `/etc/dicerd/config.yaml`, or the file
...
api:
  ...
  socket:
    ...
    #
    # Group of the socket, by name or ID. Its members can use the API as far as
    # mode lets the group, which is as much as root on this host. Empty leaves
...
#
# Credentials for private registries, by host, as an image's name gives it:
# docker.io, ghcr.io, or a registry's host:port. Images from any other registry
# are pulled anonymously. Each takes a username and a password, or a
# password_file readable only by root, or instead a credential_helper: the
# name of a docker-credential-<name> program on the daemon's PATH, for a
# registry whose tokens expire.
#
registries:
  # docker.io:
  #   username: dicer-bot
