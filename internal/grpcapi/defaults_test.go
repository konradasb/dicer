...
func TestDefaultsFallBackToTheOnlyOne(t *testing.T) {
	...
	if _, err := r.resolveKernel(""); !errors.Is(err, errdefs.ErrInvalidState) ||
		!strings.Contains(err.Error(), "none has been imported") {
		t.Errorf("resolveKernel with no kernels = %v", err)
	}

	if err := definitions.CreateKernel(types.Kernel{ID: "k-1", Name: "k1"}); err != nil {
		t.Fatal(err)
	}
	...
	if err := definitions.CreateKernel(types.Kernel{ID: "k-2", Name: "k2"}); err != nil {
		t.Fatal(err)
	}
	if _, err := r.resolveKernel(""); !errors.Is(err, errdefs.ErrInvalidArgument) ||
