...
// ExecInstance opens a shell or runs a command inside a running instance by
// proxying to dicer-agent over the instance's vsock.
func (h *instanceHandler) ExecInstance(stream execClientStream) error {
	...
	start := req.GetStart()
	if start == nil {
		return status.Error(codes.InvalidArgument, "first message must be an ExecInstanceStart")
	}
	...
	if err != nil {
		return status.Errorf(codes.Unavailable, "open guest exec stream: %v", err)
	}
	...
	if err != nil {
		return status.Errorf(codes.Internal, "send start to guest: %v", err)
	}
...
// proxyExec shuttles stdin and resize events to the guest agent and its
// output back, returning when the guest closes the stream or the client
// disconnects.
...
// until the agent closes the stream, which is a clean end.
...
// toAgentExecRequest translates a client's exec message for the agent, or
// returns nil for one the agent has no use for.
...
// toClientExecResponse translates an agent's exec message for the client,
// or returns nil for one the client has no use for.
