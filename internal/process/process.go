...
// Package process supervises long-lived processes the daemon is responsible
// for -- in practice, hypervisors.
//
// A Process is a handle on one such process, whether the daemon started it or
// adopted it after a restart. Either way the handle reports the moment the
// process exits, which is what lets the daemon notice a VMM that died without
// being asked to, rather than finding out the next time someone touches it.
package process
