...
// Attach adopts a running process the daemon did not start -- a VMM that
// outlived the previous daemon.
//
// A bare PID is not an identity: the process it named may have exited and
// the number been handed to something else. Attach therefore pins the
// process with a pidfd first, then checks that it was started with arg on
// its command line; a VMM is given its instance's API socket path, which no
// other process has. Once the check passes the pidfd refers to that process
// and no other, so neither the exit notification nor Kill can reach a
...
// cmdlineRetries and cmdlineRetryInterval bound the wait for a command line
// to appear. A process reads with an empty one for a moment while it is
// being exec'd, and for good once it is a zombie; only the first is worth
// waiting for, and briefly.
const (
	cmdlineRetries       = 20
