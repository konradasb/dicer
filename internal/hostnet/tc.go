...
// ensureBridgeQdisc creates the root HTB qdisc and root class on the given
// bridge so per-TAP upload limits can be attached beneath it.
// Idempotent: returns nil if an HTB qdisc already exists.
// A zero or negative capacityBps is a no-op.
