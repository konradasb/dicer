...
func (p *printableSnapshot) KV() []map[string]any {
	...
	for _, s := range p.Snapshots {
		kv = append(kv, map[string]any{
			...
			"Memory":     size(s.MemoryBytes),
			"Size":       size(s.SizeBytes),
...
func newSnapshotCreateCommand() *cobra.Command {
	return &cobra.Command{
		...
		Aliases:           []string{"new"},
		ValidArgsFunction: complete(1, instancesIn(dicer.StateRunning, dicer.StatePaused)),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runTask(cmd, "Snapshotting "+args[0], func() (dicer.Snapshot, error) {
				return client.CreateSnapshot(cmd.Context(), args[0], name)
			}, func(snap dicer.Snapshot, took string) string {
				return fmt.Sprintf("Snapshot %s of instance %s created in %s (%s)",
					snap.Name, snap.InstanceName, took, size(snap.SizeBytes))
			})
		},
	}
}
...
func newSnapshotListCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			snapshots, err := client.ListSnapshots(cmd.Context(), args[0])
			if err != nil {
				return err
			}

			return render(cmd, &printableSnapshot{Snapshots: snapshots})
		},
	}
...
func newSnapshotShowCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			snap, err := client.GetSnapshot(cmd.Context(), args[0], args[1])
			if err != nil {
				return err
			}

			return render(cmd, &printableSnapshot{Snapshots: []dicer.Snapshot{snap}})
		},
	}
...
func newSnapshotDeleteCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := client.DeleteSnapshot(cmd.Context(), args[0], args[1]); err != nil {
				return err
			}
