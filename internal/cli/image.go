...
func (p *printableImage) KV() []map[string]any {
	...
	for _, img := range p.Images {
		kv = append(kv, map[string]any{
			"Name":      img.Name,
			"Digest":    img.Digest,
			"Size":      size(img.SizeBytes),
			"Created":   age(img.CreatedAt),
			"Last used": age(img.LastUsedAt),
		})
	}
	return kv
}
...
func newImagePullCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			start := time.Now()
			img, err := client.PullImage(cmd.Context(), args[0], reporter.report)
			if err != nil {
				return err
			}
			reporter.done()
			...
			if !reporter.fetched {
				succeeded(cmd, "Image %s is up to date (%s)", img.Name, shortDigest(img.Digest))

				return nil
			}
			succeeded(cmd, "Image %s pulled in %s (%s, %s)", img.Name,
				formatDuration(time.Since(start)), shortDigest(img.Digest), size(img.SizeBytes))
...
func newImageListCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return render(cmd, &printableImage{Images: images})
		},
	}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return eachName(cmd, args, listImages, func(client *dicer.Client, ref string) error {
				if err := client.DeleteImage(cmd.Context(), ref, force); err != nil {
...
func newImagePruneCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			result, err := client.PruneImages(cmd.Context())
			if err != nil {
				return err
			}

			for _, img := range result.Images {
				succeeded(cmd, "Deleted %s", img.Name)
			}
			succeeded(cmd, "Reclaimed %s from %d image(s)",
				size(result.ReclaimedBytes), len(result.Images))
