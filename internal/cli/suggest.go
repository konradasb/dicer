...
func suggest(
	...
	if !ok || s.Code() != codes.NotFound || name == "" {
		return err
	}
	...
	return status.Error(codes.NotFound, s.Message()+" (did you mean "+orList(matches)+"?)")
}
...
func closeNames(name string, names []string) []string {
	...
	// A slip is an edit for every three characters or so: one in "wbe",
	// two in "dtabase". A name held in another counts too, unless it is
	// too short to mean anything by it.
...
// withHint adds what to do about an error to its message, if it has the
// code the hint is for: `instance "web" is running: stop it first or use
// -f`. The daemon says what is wrong; only the command line knows which of
// its flags would put it right.
func withHint(err error, code codes.Code, hint string) error {
	s, ok := status.FromError(err)
	if !ok || s.Code() != code {
		return err
	}
	return status.Error(code, s.Message()+": "+hint)
}
