...
func TestInstanceStatusForAnInstanceThatEnded(t *testing.T) {
	...
	status := func(s dicer.InstanceStatus) dicer.Instance { return dicer.Instance{Status: s} }
	...
	tests := []struct {
		...
		{
			status(dicer.InstanceStatus{State: dicer.StateStopped, ExitCode: code(0), FinishedAt: ago}),
			"Exited (0) 2 minutes ago",
		},
		{
			status(dicer.InstanceStatus{
				State: dicer.StateFailed, StateError: "exit code 1", ExitCode: code(1), FinishedAt: ago,
			}),
			"Exited (1) 2 minutes ago",
		},
		{
			status(dicer.InstanceStatus{State: dicer.StateFailed, StateError: "the guest reset"}),
			"Failed: the guest reset",
		},
		{
			status(dicer.InstanceStatus{State: dicer.StateRestarting, RestartCount: 3, NextRestartAt: soon}),
			"Restarting (3) in 2",
		},
		{status(dicer.InstanceStatus{State: dicer.StateRestarting, RestartCount: 1}), "Restarting (1)"},
		{status(dicer.InstanceStatus{State: dicer.StateStopped}), "Stopped"},
	}
	for _, tt := range tests {
		...
		if got := instanceStatus(tt.inst); !strings.HasPrefix(got, tt.want) ||
			(tt.inst.Status.NextRestartAt.IsZero() && got != tt.want) {
...
func TestParseRestartPolicy(t *testing.T) {
	...
	if err != nil || p.Mode != dicer.RestartOnFailure || p.MaxRetries != 5 {
