...
func runEventsCommand(cmd *cobra.Command, _ []string) error {
	...
	opts := dicer.EventOptions{
		Filter: dicer.EventFilter{Kind: dicer.EventKind(kind)},
		Limit:  int(tail),
		Follow: follow,
	}
	if name != "" {
		opts.Filter.Names = []string{name}
	}
	if sinceFlag != "" {
	...
	// The history is laid out as a whole, so it is gathered before any of
	// it is shown; what follows it is shown as it comes, widening the
	// columns if it must.
	...
	err = client.Events(cmd.Context(), opts,
		func(e dicer.Event) {
			if writeErr != nil {
				return
			}
			...
			writeErr = table.write([]dicer.Event{e})
		},
		func() {
			caughtUp = true
			if writeErr == nil {
...
func (t *eventTable) write(list []dicer.Event) error {
	for _, e := range list {
		t.kind = max(t.kind, width(eventLabel(string(e.Kind))))
		t.name = max(t.name, width(eventName(e)))
		t.action = max(t.action, width(eventLabel(string(e.Action))))
	}
	for _, e := range list {
...
func (t *eventTable) writeOne(e dicer.Event) error {
	...
	line := e.Time.Local().Format(time.DateTime) + "  " +
		pad(eventLabel(string(e.Kind)), t.kind) + "  " +
...
func eventName(e dicer.Event) string {
	name := e.Name
	if e.Kind == dicer.KindImage {
		name = reference.Familiar(name)
	}
	return shorten(name, maxNameWidth)
}

// shorten cuts s to n characters by taking out its middle, where a name
// says least: ghcr.io/org/some-very-long-image-name:1.2 keeps its registry
// and its tag.
...
// eventAction is an event's action, as a person reads it and coloured by
// what it says, then its message, if it has one, lined up after actions of
// the given width.
func eventAction(e dicer.Event, width int, p palette) string {
	action := eventLabel(string(e.Action))
	if e.Message == "" {
		return p.event(string(e.Action), action)
	}

	return p.event(string(e.Action), pad(action, width)) + "  " + e.Message
}

// eventLabel is a kind or an action as a person reads it: "snapshot_created"
// is "Snapshot created". The API's own is what scripts match on, and what
// --format json keeps.
...
func recentEvents(ctx context.Context, client *dicer.Client, instanceID string) ([]dicer.Event, error) {
	...
	err := client.Events(ctx, dicer.EventOptions{
		Filter: dicer.EventFilter{Kind: dicer.KindInstance, ID: instanceID},
		Limit:  inspectEvents,
	}, func(e dicer.Event) { out = append(out, e) }, nil)
	switch {
