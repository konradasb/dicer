...
// serve its API on socketPath. args must make it serve there; see Starter for
// why socketPath has to appear among them. It returns a handle that reports
// when the process exits.
//
// The process is deliberately not bound to ctx: a VMM has to outlive the
// request that started it, and the daemon too. ctx bounds only the wait for
// the socket; if that fails, the process is killed and the error carries
// whatever the VMM logged.
