...
func (c HealthCheck) Validate() error {
	...
	if c.HTTP != nil {
		...
		if c.HTTP.Path != "" && !strings.HasPrefix(c.HTTP.Path, "/") {
			return errdefs.InvalidArgument("health check path %q must begin with /", c.HTTP.Path)
		}
	}
	if c.TCP != nil {
		probes++
	...
	if probes != 1 {
		return errdefs.InvalidArgument("a health check needs exactly one probe: a command, an HTTP port or a TCP port")
	}

	switch {
	case c.Interval < 0, c.Timeout < 0, c.StartPeriod < 0:
		return errdefs.InvalidArgument("health check durations cannot be negative")
	case c.Retries < 0:
		return errdefs.InvalidArgument("health check retries cannot be negative")
	}
