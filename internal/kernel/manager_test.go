...
func TestPathFetchesOnceAndThenCaches(t *testing.T) {
	...
	k := dicer.Kernel{ID: "k1", Name: "test", URL: "https://example.invalid/vmlinux"}
...
func TestPathVerifiesChecksum(t *testing.T) {
	...
	k := dicer.Kernel{ID: "k1", Name: "test", URL: "https://example.invalid/vmlinux", SHA256: sha256Of("vmlinux")}
...
func TestPathWithoutURL(t *testing.T) {
	...
	if _, err := c.Path(context.Background(), dicer.Kernel{ID: "k1", Name: "test"}); err == nil {
