...
// Package volume provisions the persistent disks instances mount.
//
// A volume is a sparse file with a filesystem on it, kept on the host and
// mounted by whichever instance asks for it. It outlives the instances that
// use it: deleting an instance leaves its volumes alone, and a volume is
// removed only when it is asked for and nothing mounts it.
package volume
...
// dicer.Volume definition lives in the definition store, and its disk is found by
// ID.
type Manager struct {
	dataDir string
	logger  *slog.Logger
...
// fresh ID. Recording the returned dicer.Volume is the caller's job.
func (m *Manager) Create(ctx context.Context, name string, sizeBytes int64) (*dicer.Volume, error) {
	if sizeBytes <= 0 {
