...
func TestValidateNamesAnUnknownKey(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "log_lvl") || !strings.Contains(err.Error(), "line 2") {
...
// A registry's password file is only read at a pull; validate reads it now.
func TestValidateReadsTheRegistryPasswordFile(t *testing.T) {
	missing := filepath.Join(t.TempDir(), "token")
	path := writeConfig(t, "registries:\n  ghcr.io:\n    username: bot\n    password_file: "+missing+"\n")

	if _, err := runValidate(t, path); err == nil {
		t.Error("validate accepted a password file that does not exist")
	}
}
