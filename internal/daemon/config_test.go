...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{name: "advertised addresses", mutate: func(c *Config) {
			c.API.TCP.Listen = "0.0.0.0:7443"
			c.API.TCP.Advertise = []string{"dicer.example.com:443"}
		}},
		{name: "advertised address without port", mutate: func(c *Config) {
			c.API.TCP.Listen = "0.0.0.0:7443"
			c.API.TCP.Advertise = []string{"dicer.example.com"}
		}, wantErr: true},
		{name: "advertised without listening", mutate: func(c *Config) {
			c.API.TCP.Advertise = []string{"dicer.example.com:443"}
		}, wantErr: true},
	}
...
`)
	...
	// Unset fields keep their defaults rather than becoming empty.
	if cfg.Metrics.Path != defaultMetricsPath {
		t.Errorf("Metrics.Path = %q, want %q", cfg.Metrics.Path, defaultMetricsPath)
	}
}
...
func TestValidateRejectsBadMetricsConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid",
			metrics: MetricsConfig{Enabled: true, Listen: "127.0.0.1:9101", Path: "/metrics"},
		},
		{
		...
		},
		{
			...
			metrics: MetricsConfig{Enabled: true, Listen: "127.0.0.1", Path: "/metrics"},
			wantErr: true,
		},
		{
...
func TestResourcesDefaults(t *testing.T) {
	...
	want := dicer.Resources{VCPUs: 16, MemoryBytes: 31 << 30}
