...
func TestDriversCoverEverySupportedType(t *testing.T) {
	...
	for _, hvType := range dicer.HypervisorTypes() {
		if _, ok := d[hvType]; !ok {
			t.Errorf("no driver for supported hypervisor %q", hvType)
		}
	}
	if len(d) != len(dicer.HypervisorTypes()) {
		t.Errorf("drivers() has %d entries for %d supported types", len(d), len(dicer.HypervisorTypes()))
	}
}
