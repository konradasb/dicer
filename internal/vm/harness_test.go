...
// if need be.
func seedInstance(t *testing.T, definitions *fakeDefinitions, name string) dicer.InstanceSpec {
	t.Helper()

	if _, ok := definitions.networks["default"]; !ok {
		definitions.networks["default"] = dicer.Network{
...
func newHarness(t *testing.T) *harness {
	...
	definitions.kernels["k"] = dicer.Kernel{Name: "k"}
	...
	mgr.starters = map[dicer.HypervisorType][]hypervisor.Starter{
		dicer.HypervisorCloudHypervisor: {starter},
	}
...
func (h *harness) runtime(t *testing.T) dicer.InstanceStatus {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("dicer.InstanceStatus: %v", err)
	}
	return rt
}

// waitForState polls until the instance reaches want.
func (h *harness) waitForState(t *testing.T, want dicer.InstanceState) dicer.InstanceStatus {
	t.Helper()
...
func (h *harness) setRestart(t *testing.T, p dicer.RestartPolicy) {
	t.Helper()
...
func forceState(t *testing.T, mgr *Manager, instanceID string, state dicer.InstanceState) {
	t.Helper()
