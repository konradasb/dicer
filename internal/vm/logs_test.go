...
func writeGuestLog(t *testing.T, mgr *Manager, inst dicer.InstanceSpec, contents string) string {
	t.Helper()
...
func TestStreamLogsFollowStopsWithInstance(t *testing.T) {
	...
	pid := os.Getpid()
	if err := mgr.writeRuntime(dicer.InstanceStatus{InstanceID: inst.ID, State: dicer.StateRunning, HypervisorPID: &pid}); err != nil {
		t.Fatal(err)
	}
	...
	forceState(t, mgr, inst.ID, dicer.StateStopping)
	forceState(t, mgr, inst.ID, dicer.StateStopped)
