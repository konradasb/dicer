...
type fakeDefinitions struct {
	mu        sync.Mutex
	instances map[string]dicer.InstanceSpec
	networks  map[string]dicer.Network
	kernels   map[string]dicer.Kernel
	volumes   map[string]dicer.Volume
	dir       string
}

func newFakeDefinitions(dir string) *fakeDefinitions {
	return &fakeDefinitions{
		instances: make(map[string]dicer.InstanceSpec),
		networks:  make(map[string]dicer.Network),
		kernels:   make(map[string]dicer.Kernel),
		volumes:   make(map[string]dicer.Volume),
		dir:       dir,
	}
}

func (f *fakeDefinitions) GetInstance(nameOrID string) (dicer.InstanceSpec, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeDefinitions) getInstance(nameOrID string) (dicer.InstanceSpec, error) {
	...
	return dicer.InstanceSpec{}, fmt.Errorf("%q: %w", nameOrID, dicer.ErrNotFound)
}

func (f *fakeDefinitions) ListInstances() ([]dicer.InstanceSpec, error) {
	f.mu.Lock()
	defer f.mu.Unlock()

	names := slices.Sorted(maps.Keys(f.instances))
	out := make([]dicer.InstanceSpec, 0, len(names))
	for _, n := range names {
...
func (f *fakeDefinitions) CreateInstance(inst dicer.InstanceSpec) error {
	f.mu.Lock()
	defer f.mu.Unlock()

	if _, ok := f.instances[inst.Name]; ok {
		return dicer.Exists("instance %q already exists", inst.Name)
	}
	f.instances[inst.Name] = inst
	return nil
}

func (f *fakeDefinitions) UpdateInstance(inst dicer.InstanceSpec) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeDefinitions) RenameInstance(nameOrID string, renamed dicer.InstanceSpec) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	if _, taken := f.instances[renamed.Name]; taken && renamed.Name != current.Name {
		return dicer.Exists("instance %q already exists", renamed.Name)
	}
...
func (f *fakeDefinitions) GetNetwork(nameOrID string) (dicer.Network, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	return dicer.Network{}, fmt.Errorf("%q: %w", nameOrID, dicer.ErrNotFound)
}

func (f *fakeDefinitions) ListNetworks() ([]dicer.Network, error) {
	f.mu.Lock()
	defer f.mu.Unlock()

	names := slices.Sorted(maps.Keys(f.networks))
	out := make([]dicer.Network, 0, len(names))
	for _, n := range names {
...
func (f *fakeDefinitions) GetKernel(nameOrID string) (dicer.Kernel, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	return dicer.Kernel{}, fmt.Errorf("%q: %w", nameOrID, dicer.ErrNotFound)
}

func (f *fakeDefinitions) GetVolume(nameOrID string) (dicer.Volume, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	return dicer.Volume{}, fmt.Errorf("%q: %w", nameOrID, dicer.ErrNotFound)
}

// fakeAddresses is an in-memory Addresses.
type fakeAddresses struct {
	byNetwork map[string][]dicer.NetworkAllocation
	next      int
}

func newFakeAddresses() *fakeAddresses {
	return &fakeAddresses{byNetwork: make(map[string][]dicer.NetworkAllocation)}
}

func (f *fakeAddresses) Allocate(n dicer.Network, instanceID, staticIP string) (dicer.NetworkAllocation, error) {
...
func (f *fakeAddresses) Get(networkName, instanceID string) (dicer.NetworkAllocation, error) {
	for _, alloc := range f.byNetwork[networkName] {
		if alloc.InstanceID == instanceID {
			return alloc, nil
		}
	}
	return dicer.NetworkAllocation{}, fmt.Errorf("%q: %w", instanceID, dicer.ErrNotFound)
}
...
func (f *fakeAddresses) Reconcile(networks []string, live map[string]struct{}) (int, error) {
	...
	for _, n := range networks {
		kept := make([]dicer.NetworkAllocation, 0, len(f.byNetwork[n]))
...
func (f *fakeHostNetwork) SetupBridge(_ context.Context, n *dicer.Network) error {
	f.setUpBridges = append(f.setUpBridges, n.Bridge)
	return nil
}

func (f *fakeHostNetwork) CreateTAP(
	_ context.Context, _ *dicer.Network, _ *dicer.NetworkAllocation, _ network.Bandwidth,
) error {
	return nil
}

func (f *fakeHostNetwork) RemoveTAP(_ context.Context, _ *dicer.Network, instanceID string) {
	f.removedTAPs = append(f.removedTAPs, instanceID)
}

func (f *fakeHostNetwork) PublishPorts(
	_ context.Context, _ *dicer.Network, alloc *dicer.NetworkAllocation, ports []dicer.PortMapping,
) error {
	if f.publishErr != nil {
		return f.publishErr
	}
...
func (f *fakeHostNetwork) TeardownBridge(ctx context.Context, n *dicer.Network) {
	if ctx.Err() != nil {
		f.cancelledTeardowns.Add(1)
	}
...
func (f *fakeImages) Get(ref string) (*dicer.Image, error) {
	if f.held == nil {
...
func (f *fakeImages) Pull(context.Context, string, image.ProgressFunc) (*dicer.Image, error) {
	f.pulls++
	return &dicer.Image{
...
func (f fakeKernels) Path(context.Context, dicer.Kernel) (string, error) { return f.path, nil }
...
func (f *fakeProbe) probe(context.Context, string, dicer.HealthCheck) (probeResult, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeEvents) Record(e dicer.Event) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeEvents) undescribed() []dicer.Event {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeEvents) actions() []dicer.EventAction {
	f.mu.Lock()
	defer f.mu.Unlock()

	out := make([]dicer.EventAction, 0, len(f.events))
	for _, e := range f.events {
...
func (f *fakeEvents) last(action dicer.EventAction) (dicer.Event, bool) {
	f.mu.Lock()
	defer f.mu.Unlock()
