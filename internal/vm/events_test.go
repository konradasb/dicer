...
func (h *harness) waitForAction(t *testing.T, action dicer.EventAction) dicer.Event {
	t.Helper()
...
func TestLifecycleIsRecorded(t *testing.T) {
	...
	created := dicer.InstanceSpec{ID: "new-id", Name: "new", ImageRef: "alpine"}
	...
	want := []dicer.EventAction{
		dicer.ActionCreated, dicer.ActionStarted, dicer.ActionPaused, dicer.ActionResumed,
		dicer.ActionStopped, dicer.ActionUpdated, dicer.ActionDeleted,
	}
	...
	stopped, _ := h.events.last(dicer.ActionStopped)
	...
	first, _ := h.events.last(dicer.ActionCreated)
	if first.Kind != dicer.KindInstance || first.ID != "new-id" || first.Name != "new" {
		t.Errorf("created = %+v, want the new instance", first)
	}
	started, _ := h.events.last(dicer.ActionStarted)
...
func TestCrashAndRestartAreRecorded(t *testing.T) {
	...
	restarted, _ := h.events.last(dicer.ActionStarted)

	died, _ := h.events.last(dicer.ActionDied)
	if !strings.Contains(died.Message, "exited unexpectedly") {
		t.Errorf("died = %q, want why", died.Message)
	}
	restarting, _ := h.events.last(dicer.ActionRestarting)
...
func TestFailedStartIsRecorded(t *testing.T) {
	...
	died, ok := h.events.last(dicer.ActionDied)
...
func TestHealthVerdictsAreRecorded(t *testing.T) {
	h, probe := monitored(t, dicer.RestartPolicy{}, false)
	h.start(t)

	unhealthy := h.waitForAction(t, dicer.ActionUnhealthy)
	if !strings.Contains(unhealthy.Message, "connection refused") {
		t.Errorf("unhealthy = %q, want what the probe said", unhealthy.Message)
	}

	probe.set(true, nil)
	healthy := h.waitForAction(t, dicer.ActionHealthy)
...
func TestSnapshotsAreRecorded(t *testing.T) {
	...
	created, _ := h.events.last(dicer.ActionSnapshotCreated)
	deleted, _ := h.events.last(dicer.ActionSnapshotDeleted)
...
func TestStopMessage(t *testing.T) {
	...
	tests := []struct {
		...
		{stopNotRunning, "dicer.InstanceSpec was not running; nothing to stop"},
	}
...
func TestUpdateMessage(t *testing.T) {
	before := dicer.InstanceSpec{ImageRef: "docker.io/library/nginx:1.27", VCPUs: 1, MemoryBytes: 256 << 20, DiskBytes: 10 << 30}
	after := before
	...
	if got := updateMessage(before, after, dicer.StateStopped); got != want {
		t.Errorf("updateMessage =\n%q\nwant\n%q", got, want)
	}

	after = before
	after.Restart = dicer.RestartPolicy{Mode: dicer.RestartOnFailure, MaxRetries: 3}
	want = "Updated instance: restart policy no → on-failure:3; takes effect when the instance next ends"
	if got := updateMessage(before, after, dicer.StateRunning); got != want {
		t.Errorf("updateMessage =\n%q\nwant\n%q", got, want)
	}
}
