...
var quickCheck = dicer.HealthCheck{
	Exec:     []string{"true"},
...
func monitored(t *testing.T, restart dicer.RestartPolicy, healthy bool) (*harness, *fakeProbe) {
	t.Helper()
...
func (h *harness) waitForHealth(t *testing.T, want dicer.HealthStatus) dicer.Health {
	t.Helper()
...
func TestHealthyInstanceIsReportedHealthy(t *testing.T) {
	h, _ := monitored(t, dicer.RestartPolicy{}, true)
	h.start(t)

	state := h.waitForHealth(t, dicer.HealthHealthy)
...
// dicer.Usage counts checked instances by verdict, naming every verdict.
func TestUsageCountsHealth(t *testing.T) {
	h, _ := monitored(t, dicer.RestartPolicy{}, true)
	h.start(t)
	h.waitForHealth(t, dicer.HealthHealthy)

	got := h.mgr.Usage().ByHealth
	want := map[dicer.HealthStatus]int{dicer.HealthStarting: 0, dicer.HealthHealthy: 1, dicer.HealthUnhealthy: 0}
...
func TestUnhealthyInstanceIsRestarted(t *testing.T) {
	h, probe := monitored(t, dicer.RestartPolicy{Mode: dicer.RestartAlways}, false)
	h.restartAtOnce()
	h.start(t)
	first := h.starter.vmm()
...
func TestUnhealthyInstanceWithoutRestartPolicyKeepsRunning(t *testing.T) {
	for name, policy := range map[string]dicer.RestartPolicy{
		"no":                   {Mode: dicer.RestartNo},
		"retries already used": {Mode: dicer.RestartOnFailure, MaxRetries: 1},
	} {
		t.Run(name, func(t *testing.T) {
			...
			state := h.waitForHealth(t, dicer.HealthUnhealthy)
			...
			if rt := h.runtime(t); rt.State != dicer.StateRunning || len(h.starter.vmms) != 1 {
...
func TestStopEndsHealthChecks(t *testing.T) {
	h, probe := monitored(t, dicer.RestartPolicy{}, true)
	h.start(t)
	h.waitForHealth(t, dicer.HealthHealthy)
...
func TestPausedInstanceIsNotProbed(t *testing.T) {
	...
	if rt := h.runtime(t); rt.State != dicer.StatePaused {
...
func TestOutdatedAgentIsNotUnhealthy(t *testing.T) {
	...
	if _, state, _ := h.mgr.Health(h.inst); state.Status != dicer.HealthStarting {
...
func TestHealthCheckComesFromTheImage(t *testing.T) {
	imageCheck := &dicer.HealthCheck{TCP: &dicer.TCPProbe{Port: 5432}}

	for name, tt := range map[string]struct {
		own       *dicer.HealthCheck
		monitored bool
	}{
		"inherited":    {own: nil, monitored: true},
		"switched off": {own: &dicer.HealthCheck{Disabled: true}, monitored: false},
	} {
		t.Run(name, func(t *testing.T) {
			h := newHarness(t)
			...
			if ok && (check.TCP == nil || check.Interval != dicer.DefaultHealthInterval) {
...
func TestObserve(t *testing.T) {
	check := dicer.HealthCheck{Retries: 3, StartPeriod: time.Minute}
	at := time.Now()
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			sinceStart: time.Hour,
			wantStatus: dicer.HealthUnhealthy,
			wantStreak: 3,
		},
		{
...
func TestObserveTakesASuccessInTheStartPeriod(t *testing.T) {
	health := observe(dicer.NewHealth(), dicer.HealthCheck{Retries: 3, StartPeriod: time.Hour},
		probeResult{Healthy: true, At: time.Now()}, time.Second)

	if health.Status != dicer.HealthHealthy {
...
func TestObserveKeepsWhatTheProbeSaid(t *testing.T) {
	at := time.Now()
	health := observe(dicer.NewHealth(), dicer.HealthCheck{Retries: 1},
		probeResult{Output: strings.Repeat("é", maxOutput), At: at}, time.Hour)

	if len(health.LastOutput) > maxOutput {
		t.Errorf("output is %d bytes, want at most %d", len(health.LastOutput), maxOutput)
	}
