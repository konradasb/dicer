...
func TestRecoverAdoptsLiveInstance(t *testing.T) {
	...
	err := mgr.writeRuntime(dicer.InstanceStatus{
		InstanceID:    inst.ID,
...
func TestAdoptedVMMCrashFailsInstance(t *testing.T) {
	...
	if err := mgr.writeRuntime(dicer.InstanceStatus{
		InstanceID: inst.ID, State: dicer.StateRunning, HypervisorPID: &pid,
	}); err != nil {
		t.Fatalf("writeRuntime: %v", err)
	}
...
func TestRecoverKillsVMMOfInterruptedStop(t *testing.T) {
	...
	pid := vmm.PID()
	if err := mgr.writeRuntime(dicer.InstanceStatus{
		InstanceID: inst.ID, State: dicer.StateStopping, HypervisorPID: &pid,
	}); err != nil {
		t.Fatalf("writeRuntime: %v", err)
	}
	...
	if rt.State != dicer.StateFailed || !strings.Contains(rt.StateError, "interrupted") {
...
func TestRecoverCleansUpDeadInstance(t *testing.T) {
	...
	if rt.State != dicer.StateFailed || !strings.Contains(rt.StateError, "not running") {
...
func TestRecoverCleansUpInterruptedStart(t *testing.T) {
	...
	inst := seedInstance(t, definitions, "web")
	forceState(t, mgr, inst.ID, dicer.StateStarting)
	...
	if rt.State != dicer.StateFailed || !strings.Contains(rt.StateError, "start interrupted") {
...
func TestRecoverKeepsBridgeWhileAnotherInstanceRuns(t *testing.T) {
	...
	deadPID := deadPID(t)
	if err := mgr.writeRuntime(dicer.InstanceStatus{
		InstanceID: dead.ID, State: dicer.StateRunning, HypervisorPID: &deadPID,
	}); err != nil {
		t.Fatalf("writeRuntime: %v", err)
	}
	...
	if err := mgr.writeRuntime(dicer.InstanceStatus{
		InstanceID: live.ID, State: dicer.StateRunning, HypervisorPID: &livePID,
	}); err != nil {
		t.Fatalf("writeRuntime: %v", err)
	}
...
func TestStartOnBootOnlyStartsInstancesThatAskToBeRunning(t *testing.T) {
	...
	onFailure.Restart = dicer.RestartPolicy{Mode: dicer.RestartOnFailure}
	definitions.instances[onFailure.Name] = onFailure
	stopped := seedInstance(t, definitions, "cache")
	stopped.Restart = dicer.RestartPolicy{Mode: dicer.RestartUnlessStopped}
...
func TestStartOnBootStartsFailedInstance(t *testing.T) {
	for _, tt := range []struct {
		policy        dicer.RestartMode
		stoppedByUser bool
	}{
		{dicer.RestartUnlessStopped, false},
		// always overrides a user's stop.
		{dicer.RestartAlways, true},
	} {
		t.Run(string(tt.policy), func(t *testing.T) {
			h := newHarness(t)
			h.setRestart(t, dicer.RestartPolicy{Mode: tt.policy})
