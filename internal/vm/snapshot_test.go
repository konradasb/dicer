...
func TestCreateSnapshotOfPausedInstance(t *testing.T) {
	h := newHarness(t)
	h.running(t)
	forceState(t, h.mgr, h.inst.ID, dicer.StatePaused)
	...
	if rt, _ := h.mgr.Runtime(h.inst); rt.State != dicer.StatePaused {
		t.Errorf("state = %s, want it left %s", rt.State, dicer.StatePaused)
	}
}
...
func TestListAndDeleteSnapshots(t *testing.T) {
	...
	if _, err := h.mgr.GetSnapshot(h.inst, "first"); !errors.Is(err, dicer.ErrNotFound) {
		t.Errorf("GetSnapshot after delete = %v, want ErrNotFound", err)
	}
	if err := h.mgr.DeleteSnapshot(t.Context(), h.inst, "first"); !errors.Is(err, dicer.ErrNotFound) {
...
func TestRestoreSnapshot(t *testing.T) {
	...
	if rt.State != dicer.StateRunning {
		t.Errorf("state = %s, want %s", rt.State, dicer.StateRunning)
	}
...
func TestRestoreSnapshotRejections(t *testing.T) {
	...
	t.Run("hypervisor version gone", func(t *testing.T) {
		...
		if err == nil || !errors.Is(err, dicer.ErrInvalidState) {
			t.Errorf("RestoreSnapshot = %v, want a complaint about the missing version", err)
		}
		if rt, _ := h.mgr.Runtime(h.inst); rt.State != dicer.StateStopped {
			t.Errorf("state = %s, want the instance left %s", rt.State, dicer.StateStopped)
		}
	})
}
...
func TestRestoreSnapshotFailureMarksFailed(t *testing.T) {
	...
	if rt.State != dicer.StateFailed {
		t.Errorf("state = %s, want %s", rt.State, dicer.StateFailed)
	}
	if rt.StateError == "" {
...
func TestCreateSnapshotRecordsWhatTheGuestHas(t *testing.T) {
	h := newHarness(t)
	h.running(t)
	if err := h.mgr.transitionWith(h.inst, dicer.StateRunning, func(rt *dicer.InstanceStatus) {
