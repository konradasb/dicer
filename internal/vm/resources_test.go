...
func TestCapacityAllocatable(t *testing.T) {
	want := dicer.Resources{VCPUs: 16, MemoryBytes: 7 << 30}
...
func admitHarness(t *testing.T) (*harness, dicer.InstanceSpec) {
	t.Helper()
...
func holding(t *testing.T, mgr *Manager, inst dicer.InstanceSpec, state dicer.InstanceState, r dicer.Resources) {
	t.Helper()
...
func TestStartRefusedWhenTheHostIsFull(t *testing.T) {
	...
	holding(t, h.mgr, other, dicer.StateRunning, dicer.Resources{VCPUs: 1, MemoryBytes: 6 << 30})

	err := h.mgr.Start(t.Context(), h.inst)
	if !errors.Is(err, dicer.ErrResourceExhausted) {
...
func TestWhatHoldsResources(t *testing.T) {
	for _, tc := range []struct {
		...
		{dicer.StateStarting, true},
		{dicer.StateRunning, true},
		{dicer.StatePaused, true},
		{dicer.StateStopping, false},
		{dicer.StateStopped, false},
		{dicer.StateFailed, false},
	} {
		h, other := admitHarness(t)
		h.inst.VCPUs, h.inst.MemoryBytes = 1, 2<<30
		holding(t, h.mgr, other, tc.state, dicer.Resources{VCPUs: 1, MemoryBytes: 6 << 30})

		err := h.mgr.Start(t.Context(), h.inst)
		if refused := errors.Is(err, dicer.ErrResourceExhausted); refused != tc.holds {
...
func TestConcurrentStartsCannotBothTakeTheLastRoom(t *testing.T) {
	...
	holding(t, h.mgr, third, dicer.StateRunning, dicer.Resources{VCPUs: 1, MemoryBytes: 6 << 30})
	...
	for i, inst := range []dicer.InstanceSpec{h.inst, other} {
		wg.Go(func() {
	...
	for _, err := range results {
		...
		case err == nil:
			admitted++
		case !errors.Is(err, dicer.ErrResourceExhausted):
...
func TestCheckResources(t *testing.T) {
	...
	if err := mgr.CheckResources(dicer.Resources{VCPUs: 4, MemoryBytes: 7 << 30}); err != nil {
		t.Errorf("the whole host: %v", err)
	}
	// More vCPUs than CPUs is never useful to one instance, overcommit or
	// not.
	if err := mgr.CheckResources(dicer.Resources{VCPUs: 5, MemoryBytes: 1 << 30}); !errors.Is(err, dicer.ErrInvalidArgument) {
		t.Errorf("5 vCPUs on 4 CPUs: %v, want ErrInvalidArgument", err)
	}
	if err := mgr.CheckResources(dicer.Resources{VCPUs: 1, MemoryBytes: 8 << 30}); !errors.Is(err, dicer.ErrInvalidArgument) {
		t.Errorf("more memory than allocatable: %v, want ErrInvalidArgument", err)
	}

	// Without a capacity, nothing is refused.
	mgr.capacity = dicer.Capacity{}
	if err := mgr.CheckResources(dicer.Resources{VCPUs: 1000, MemoryBytes: 1 << 50}); err != nil {
