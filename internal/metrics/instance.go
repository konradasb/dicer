...
type InstanceStats struct {
	// ByState counts instances per lifecycle state. The source is expected
	// to name every state it knows, zeros included: a state that disappears
	// from the map when it empties leaves a gap in the series rather than a
	// zero, and a gap is not something PromQL can average over.
