...
// Package filestore is the store of what is defined on this host: instances,
// networks, volumes and kernels, and who is trusted to use the API.
//
// It stores definitions and nothing else. Address allocation lives in
// internal/network and runtime state in internal/vm -- each is policy
// rather than storage, and each keeps its own table.
//
// There is no consensus, no replication and no leader: one daemon owns one
// host, so the filesystem is the source of truth. Definitions are YAML files,
// readable and editable by hand:
//
//	/var/lib/dicer/instances/<name>/config.yaml
//	/var/lib/dicer/instances/<name>/overlay.img
//	/var/lib/dicer/networks/<name>.yaml
//	/var/lib/dicer/volumes/<name>.yaml
//	/var/lib/dicer/kernels/<name>.yaml
//	/var/lib/dicer/access/clients/<name>.yaml
//	/var/lib/dicer/access/tokens/<name>.yaml
//
// Everything is cached in memory at open and written through on mutation.
package filestore
...
// DefaultDataDir is where definitions live unless configured otherwise.
const DefaultDataDir = "/var/lib/dicer"
...
//
// It is the implementation of vm.Definitions and access.Store.
type Manager struct {
	dataDir string
	logger  *slog.Logger

	instances *collection[dicer.InstanceSpec]
	networks  *collection[dicer.Network]
	volumes   *collection[dicer.Volume]
	kernels   *collection[dicer.Kernel]
	clients   *collection[dicer.TrustedClient]
	tokens    *collection[dicer.AccessToken]
}

// NewManager loads all definitions from disk into memory. A missing data
// directory is created; a malformed individual file is logged and skipped
// rather than failing the whole daemon, so one bad hand-edit cannot prevent
// startup.
func NewManager(cfg Config) (*Manager, error) {
	...
	for _, dir := range []string{instancesDir, networksDir, volumesDir, kernelsDir, clientsDir, tokensDir} {
	...
	m.instances = newCollection(
		...
		func(v dicer.InstanceSpec) (string, string) { return v.ID, v.Name },
	)
	m.networks = newCollection(
		"network", filepath.Join(cfg.DataDir, networksDir), flat, logger,
		func(v dicer.Network) (string, string) { return v.ID, v.Name },
	)
	m.volumes = newCollection(
		"volume", filepath.Join(cfg.DataDir, volumesDir), flat, logger,
		func(v dicer.Volume) (string, string) { return v.ID, v.Name },
	)
	m.kernels = newCollection(
		"kernel", filepath.Join(cfg.DataDir, kernelsDir), flat, logger,
		func(v dicer.Kernel) (string, string) { return v.ID, v.Name },
	)
	// A client is known by its certificate's fingerprint, and a token by
	// its secret's hash: those are what a request presents.
	m.clients = newCollection(
		"client", filepath.Join(cfg.DataDir, clientsDir), flat, logger,
		func(v dicer.TrustedClient) (string, string) { return v.Fingerprint, v.Name },
	)
	m.tokens = newCollection(
		"token", filepath.Join(cfg.DataDir, tokensDir), flat, logger,
		func(v dicer.AccessToken) (string, string) { return v.SecretHash, v.Name },
	)
	for _, coll := range []interface{ load() error }{
		m.instances, m.networks, m.volumes, m.kernels, m.clients, m.tokens,
	} {
...
const (
	...
	volumesDir   = "volumes"
	kernelsDir   = "kernels"
	clientsDir   = "access/clients"
	tokensDir    = "access/tokens"
...
// collection is a name-keyed set of resources backed by YAML files.
//
// Both the name and the ID are accepted for lookup: names are what humans
// type and are unique per host, while IDs are stable across renames and are
// what host resources (TAP devices, runtime directories) are derived from.
...
func (c *collection[T]) get(nameOrID string) (T, error) {
	...
	if !ok {
		return zero, dicer.NotFound("no %s %q", c.kind, nameOrID)
	}
...
func (c *collection[T]) create(item T) error {
	...
	if _, ok := c.items[name]; ok {
		return dicer.Exists("%s %q already exists", c.kind, name)
	}
...
func (c *collection[T]) update(item T) error {
	...
	id, name := c.keyOf(item)
	if _, ok := c.items[name]; !ok {
		return dicer.NotFound("no %s %q", c.kind, name)
	}
...
// rename moves a resource to a new name, taking whatever else its directory
// holds with it.
//
// The resource keeps its ID, so everything derived from that -- the runtime
// directory, the TAP device, the address -- is untouched. rename is given the
// renamed item rather than renaming it here, because only the caller knows
// which of the resource's fields carry its name.
func (c *collection[T]) rename(nameOrID string, renamed T) error {
	...
	if !ok {
		return dicer.NotFound("no %s %q", c.kind, nameOrID)
	}

	id, to := c.keyOf(renamed)
	if err := dicer.ValidateName(to); err != nil {
		return err
	}
	if to == from {
		return nil
	...
	if _, taken := c.items[to]; taken {
		return dicer.Exists("%s %q already exists", c.kind, to)
	}
