...
type recordedEvents []dicer.Event

func (r *recordedEvents) Record(e dicer.Event) { *r = append(*r, e) }

func (r *recordedEvents) actions() []dicer.EventAction {
	out := make([]dicer.EventAction, 0, len(*r))
	for _, e := range *r {
...
func TestPullIsRecorded(t *testing.T) {
	...
	if got := recorded.actions(); !slices.Equal(got, []dicer.EventAction{dicer.ActionPulled}) {
	...
	if e.Kind != dicer.KindImage || e.Name != "docker.io/library/nginx:1.27" || e.Attributes["digest"] != "sha256:aaa" {
...
func TestRemovalsAreRecorded(t *testing.T) {
	...
	*recorded = nil
	...
	want := []struct {
		name   string
		action dicer.EventAction
		attr   string
		value  string
	}{
		{"docker.io/library/a:1", dicer.ActionDeleted, "by", "user"},
		{"docker.io/library/c:1", dicer.ActionCollected, "reason", "unused"},
		{"docker.io/library/b:1", dicer.ActionDeleted, "by", "prune"},
	}
