...
// Package reference parses and resolves OCI image references.
//
// It separates the two halves of the problem: parsing a reference into its
// registry, repository and tag or digest, and resolving a mutable tag to the
// immutable digest that identifies an image.
package reference
...
// Familiar returns a reference as people write it, without the registry and
// namespace they need not: docker.io/library/busybox:latest is
// busybox:latest, and docker.io/grafana/grafana:latest grafana/grafana:latest.
// A reference that does not parse is returned as it is.
