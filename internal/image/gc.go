...
// GCPolicy is when images nothing uses are removed without being asked to:
// once they have gone unused too long, and while the image store is larger
// than it may be. An image in use -- by an instance defined to boot from it,
// a running guest or a snapshot -- is never removed.
//
// The zero policy removes nothing.
...
// gcGracePeriod is how recently an image may have been used and still be
// collected: not at all. It covers a pull made for an instance that is about
// to be created, as dicer run makes one, before the instance exists to put
// the image in use.
...
func (p GCPolicy) expired(images []*dicer.Image, inUse map[string]struct{}, now time.Time) []*dicer.Image {
	if p.MaxUnusedAge <= 0 {
		return nil
	}
...
// first.
func collectable(images []*dicer.Image, inUse map[string]struct{}, now time.Time) []*dicer.Image {
	var out []*dicer.Image
	...
	slices.SortFunc(out, func(a, b *dicer.Image) int {
		return cmp.Or(a.LastUsedAt.Compare(b.LastUsedAt), cmp.Compare(a.Digest, b.Digest))
	})
	return out
}

// CollectGarbage applies the policy once, at now, to the images not in
// inUse, and records the ones in it as used now.
//
// Images unused for too long go first. Then, while the store is over its
// size, the least recently used of the rest go one at a time, the store
// measured again after each: an image's cached layers may be shared with
// another's, so what removing it frees is only known once it is gone.
func (m *Manager) CollectGarbage(p GCPolicy, inUse map[string]struct{}, now time.Time) (GCResult, error) {
	...
	for _, img := range collectable(m.index.list(), inUse, now) {
		...
		if err := m.collect(p, []*dicer.Image{img}, GCReasonSize, &result); err != nil {
			return result, err
		}
	}
...
func (m *Manager) collect(p GCPolicy, images []*dicer.Image, reason GCReason, result *GCResult) error {
	if len(images) == 0 {
		return nil
	}
	...
	for _, img := range removed.Images {
		...
		m.record(&img, dicer.ActionCollected, gcMessage(p, &img, reason), map[string]string{"reason": string(reason)})
	}
...
func gcMessage(p GCPolicy, img *dicer.Image, reason GCReason) string {
