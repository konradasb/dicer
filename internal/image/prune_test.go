...
func pullTestImage(t *testing.T, m *Manager, mock *mockRegistryClient, ref, digest string) *dicer.Image {
	t.Helper()
...
func TestPullReportsProgress(t *testing.T) {
	...
	var got []dicer.PullProgress
	if _, err := m.Pull(t.Context(), "alpine:3.21", func(p dicer.PullProgress) { got = append(got, p) }); err != nil {
		t.Fatalf("Pull: %v", err)
	}

	var stages []dicer.PullStage
	for _, p := range got {
	...
	want := []dicer.PullStage{dicer.StageResolving, dicer.StageDownloading, dicer.StageUnpacking, dicer.StageConverting}
	...
	var sawBytes bool
	for _, p := range got {
		if p.Stage == dicer.StageDownloading && p.DownloadedBytes == 60 && p.TotalBytes == 100 {
			sawBytes = true
		}
	}
	if !sawBytes {
