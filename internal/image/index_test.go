...
func TestIndex_CreateDuplicate(t *testing.T) {
	...
	if err := s.create(img); !errors.Is(err, dicer.ErrExists) {
		t.Errorf("create duplicate = %v, want %v", err, dicer.ErrExists)
	}
}
...
func TestIndex_DeleteNotFound(t *testing.T) {
	...
	if err := s.delete("sha256:notfound"); !errors.Is(err, dicer.ErrNotFound) {
		t.Errorf("delete non-existent = %v, want %v", err, dicer.ErrNotFound)
	}
}
...
func TestFindByNameReturnsTheLatestPull(t *testing.T) {
	x := newIndex()
	now := time.Now()
	older := &dicer.Image{Name: "docker.io/library/alpine:3.21", Digest: "sha256:old", CreatedAt: now.Add(-time.Hour)}
	newer := &dicer.Image{Name: "docker.io/library/alpine:3.21", Digest: "sha256:new", CreatedAt: now}
	// Indexed newest first, as a restart may load them: the pull times
	// decide, not the order they are indexed in.
	for _, img := range []*dicer.Image{newer, older} {
...
func TestIndexMarkUsed(t *testing.T) {
	...
	held := &dicer.Image{Digest: "sha256:abc", LastUsedAt: then}
