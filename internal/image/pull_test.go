...
func TestPullSurvivesAWaiterGivingUp(t *testing.T) {
	...
	go func() {
		_, err := m.Pull(first, "alpine:latest", func(dicer.PullProgress) {})
		firstErr <- err
	}()
	<-started
...
func TestProgressRelayStopsAtRemove(t *testing.T) {
	var r progressRelay
	var got []dicer.PullStage
	id := r.add(func(p dicer.PullProgress) { got = append(got, p.Stage) })

	r.send(dicer.PullProgress{Stage: dicer.StageDownloading})
	r.remove(id)
	r.send(dicer.PullProgress{Stage: dicer.StageConverting})

	if len(got) != 1 || got[0] != dicer.StageDownloading {
