...
var ErrNoAvailableIPs = dicer.ResourceExhausted("the network has no free addresses left")
...
// allocateIP picks an available IP from ipNet, trying random candidates
// before falling back to a sequential scan.
//
// The usable range is offsets 1 through subnetSize-2: the network address and
// the broadcast address are never handed out. net.IPNet.Contains is true for
// both, so the bounds have to be computed rather than discovered by walking
// until Contains fails -- doing that would assign the broadcast address as
// the last allocation on any subnet.
