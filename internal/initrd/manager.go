...
// Package initrd builds the initramfs a guest boots from.
//
// The archive carries dicer-init, which runs as PID 1 and turns a container
// image into something bootable, plus dicer-agent for exec. It is rebuilt
// only when its contents change, keyed by a hash of the embedded binaries.
package initrd
