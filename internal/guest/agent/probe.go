...
// Probe runs one health check probe within the request's timeout. A failed
// probe is reported in the response, not as an error.
...
func (b *boundedBuffer) Write(p []byte) (int, error) {
	if room := guest.MaxProbeOutput - len(b.buf); room > 0 {
