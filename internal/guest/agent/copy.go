...
func (s *server) CopyOut(
	...
	_, err := archive.Send(src, func(chunk []byte) error {
		sendErr = stream.Send(&diceragentv1.CopyOutResponse{Data: chunk})
		return sendErr
	})
	if err != nil {
		return copyError(err, sendErr)
	}
...
// copyError says why a copy failed in terms the caller can act on. streamErr
// is the stream's own error, if the stream failed, and passes through.
func copyError(err, streamErr error) error {
	if streamErr != nil {
		return streamErr
	}
