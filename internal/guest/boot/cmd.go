...
// console is where dicer-init logs: the guest's console, reopened when a
// write to it fails.
//
// An init that runs as the workload may take the console over --
// s6-linux-init does, hanging up every file already open on the terminal --
// after which a write to the file dicer-init opened at boot fails. What
// dicer-init says after that is why the guest ended, so it reopens the
// console rather than fall silent.
type console struct {
	mu   sync.Mutex
