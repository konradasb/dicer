...
// is not the first: the guest reset without reporting its end, and a VMM
// that reboots guests in place would otherwise hide that it did. The host
// zeroes the disk before every start, so a second boot can only be a reset.
//
// A guest configured with no status disk has nowhere to report and is left
// to boot.
