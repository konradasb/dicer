...
func TestResolveMode(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			cfg: guest.Config{Entrypoint: []string{"/lib/systemd/systemd"}}, want: dicer.ModeSystemd,
		},
		{
			name: "init linked to systemd", files: debian, links: map[string]string{"sbin/init": "/lib/systemd/systemd"},
			cfg: guest.Config{Entrypoint: []string{"/sbin/init"}}, want: dicer.ModeSystemd,
		},
		{
			name: "a relative link", files: debian, links: map[string]string{"sbin/init": "../lib/systemd/systemd"},
			cfg: guest.Config{Cmd: []string{"/sbin/init"}}, want: dicer.ModeSystemd,
		},
		{
		...
		{
			name: "nothing there", cfg: guest.Config{Entrypoint: []string{"missing"}}, want: dicer.ModeExec,
		},
		{
			name: "a mode asked for is kept", files: debian, links: map[string]string{"sbin/init": "/lib/systemd/systemd"},
			cfg: guest.Config{Mode: dicer.ModeExec, Entrypoint: []string{"/sbin/init"}}, want: dicer.ModeExec,
		},
	}
