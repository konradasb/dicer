...
// Status is what the guest reports of its end on the status disk: a raw disk
// holding one zero-padded JSON document, reset by the host before each boot.
type Status struct {
	// Boots counts the boots since the host reset the disk. More than one
	// means the guest reset.
